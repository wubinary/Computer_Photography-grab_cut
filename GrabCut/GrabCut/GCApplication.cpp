...
void GCApplication::showFgd() const
{
	Mat outImage((*image).size(), (*image).type());
	Point p;
	for (p.y = 0; p.y < (*image).rows; p.y++)
	{
		for (p.x = 0; p.x < (*image).cols; p.x++)
		{
			if (mask.at<uchar>(p) == GC_PR_FGD || mask.at<uchar>(p) == GC_FGD)
				outImage.at<Vec3b>(p) = (Vec3b)(*image).at<Vec3b>(p);
		}
	}
	if (rectState == IN_PROCESS || rectState == SET)
		rectangle(outImage, Point(rect.x, rect.y), Point(rect.x + rect.width, rect.y + rect.height), RED, 2);
	imshow(*winName, outImage);
}

void GCApplication::showBgd() const
{
	Mat outImage((*image).size(), (*image).type());
	Point p;
	for (p.y = 0; p.y < (*image).rows; p.y++)
	{
		for (p.x = 0; p.x < (*image).cols; p.x++)
		{
			if (mask.at<uchar>(p) == GC_PR_BGD || mask.at<uchar>(p) == GC_BGD)
				outImage.at<Vec3b>(p) = (Vec3b)(*image).at<Vec3b>(p);
		}
	}
	imshow(*winName, outImage);
...
void GCApplication::saveImage(string path)
{
	Mat outImage((*image).size(), CV_8UC4);
	Point p;
	for (p.y = 0; p.y < (*image).rows; p.y++) 
	{
		for (p.x = 0; p.x < (*image).cols; p.x++) 
		{
			...
			if (mask.at<uchar>(p) == GC_PR_FGD || mask.at<uchar>(p) == GC_FGD) {
				outImage.at<Vec4b>(p)[0] = (*image).at<Vec3b>(p)[0];
				outImage.at<Vec4b>(p)[1] = (*image).at<Vec3b>(p)[1];
				outImage.at<Vec4b>(p)[2] = (*image).at<Vec3b>(p)[2];
				outImage.at<Vec4b>(p)[3] = 255; //¤£³z©ú
			}else {
				outImage.at<Vec4b>(p)[3] = 0; //³z©ú
			}
		}
	}
	try {
		imwrite(path.c_str(), outImage);
	}
	catch (...) {
		cout << "\t[ERROR] write image !" << endl;
	}
}
